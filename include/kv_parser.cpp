...
#include "shared.hpp"
#include <format>
ValveKeyValueFormat::TokenPair ValveKeyValueFormat::VmtParser::peek() {
...
ValveKeyValueFormat::TokenPair ValveKeyValueFormat::VmtParser::advance() {
    if (m_last_peek.first != TokenTypes::EMPTY) {
...
bool ValveKeyValueFormat::VmtParser::match(ValveKeyValueFormat::TokenTypes type, bool consume) {
    auto token = peek();
    if (token.first != type) return false;
    if (consume) advance();
    return true;
}
ValveKeyValueFormat::TokenPair ValveKeyValueFormat::VmtParser::expect(ValveKeyValueFormat::TokenTypes type) {
    auto token = peek();
    if (token.first != type) {
...
void ValveKeyValueFormat::VmtParser::parse() {
    while (!match(TokenTypes::END_OF_FILE)) {
        ...
        if (match(TokenTypes::STRING)) {
            ...
            if (match(TokenTypes::LBRACE, true)) {
                auto new_branch = std::make_shared<VmtBranch>(key.second);
                ...
                if (match(TokenTypes::STRING)) {
                    ...
                    auto leaf = std::make_shared<VmtLeaf>(key.second, value.second, condition.second);
                    m_node_stack.back()->add_branch(leaf);
                } else {
                    auto leaf = std::make_shared<VmtLeaf>(key.second, value.second);
                    m_node_stack.back()->add_branch(leaf);
                }
            }
